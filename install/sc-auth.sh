...
# reads config at startup). Systemd first — pgrep matches the systemd-owned
# process too, so if we didn't check systemd first we'd kill its process and
# race the auto-restart against a user-owned fork for :7777.
