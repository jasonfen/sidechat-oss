...
const DB_PATH = Bun.env.DB_PATH ?? "/opt/sidechat/sidechat.db";
...
app.post("/register", async (c) => {
  ...
  // Validate name
  if (!body.name || typeof body.name !== "string" || body.name.length > 64 || !/^[a-zA-Z0-9-]+$/.test(body.name)) {
    return c.json({ error: "Invalid name: must be 1-64 chars, alphanumeric and hyphens only" }, 400);
  }
  ...
  if (existing) {
    ...
    if (existing.status === "revoked") return c.json({ error: "Client has been revoked" }, 403);
  }
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .create-form input:focus { border-color: #58a6ff; }
...
(function() {
  ...
  function renderActive(clients) {
    ...
    var html = '<table><tr><th>Name</th><th>Fingerprint</th><th>Can Post</th><th>Last Seen</th><th>Last IP</th><th>Actions</th></tr>';
    clients.forEach(function(c) {
      ...
      html += '<td><button class="btn btn-revoke" onclick="adminAction(\\'/admin/clients/' + c.fingerprint + '/revoke\\',refresh)">Revoke</button></td></tr>';
    });
    html += '</table>';
  ...
  document.getElementById('obs-create-btn').addEventListener('click', function() {
    ...
    fetch('/admin/observers', {
      ...
      headers: { 'Content-Type': 'application/json' },
      credentials: 'same-origin',
      body: JSON.stringify({ username: user, password: pass, confirm_password: confirm })
    })
...
app.post("/admin/observers", requireAdmin, async (c) => {
  ...
  const existing = db.query("SELECT id FROM observers WHERE username = ?").get(body.username);
